...
begin
	...
	new_size = trunc.(Int, size(large_image) .* percentage_scale)
	image = imresize(large_image, (new_size))
end
...
# ╔═╡ a039b910-dcb0-428d-ac4e-da49cec0e83e
show_kernel(kernel)
			...
			for j in 1:cols
				...
				for r_offset in -lr:lr
					for c_offset in -lc:lc
						...
						acc += extend(M, i + r_offset, j + c_offset) * K[lr+1 + r_offset, lc+1 + c_offset]
					end
				end
				M′[i,j] = clamp_to_zero_one(acc)
			end
...
manifest_format = "2.0"
project_hash = "0af3161ab8784b5d8b5b7d396f1f12ea92895938"
