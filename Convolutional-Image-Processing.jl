...
# ╔═╡ 013a47c2-f5f7-485a-b11c-da8cea16c75e
convolved_image= convolve(image, kernel1)
...
# ╔═╡ 433ec2d6-441a-4f23-a31a-8e4e59fbb815
function SeamAdding_Y(img, width)
	if width < 0 || width > size(img)[1]
		return "width not in bounds of the image"
	end
	...
	for i = (1:size(img)[1]+width)
		energy = edges(img)
		leastE_map, directions = LeastEnergyMap_Y(energy)
		top_row = leastE_map[1,:]
		position = findmin(top_row)[2]
		FindSeamAt_Y(directions, position)

		for j = (1:length(top_row)) 
			#remove the lowest value form top_row
		end
	end
end
