...
# ╔═╡ 4fc0bf9e-b620-4768-9aa2-791b554b42b7
begin
	image_to_carve = image2
	@bind width Slider(size(image_to_carve)[2]:-1:1, show_value=true)
end
...
# ╔═╡ 19574796-e7ea-4539-8820-ebbf2a2b7d0c
show_kernel(kernel)
...
# ╔═╡ b2b3340d-d980-4235-a205-203803191930
convolvedImage = convolve(image, kernel1)

# ╔═╡ 8d15c385-44de-4117-a611-75add5a129d6
[image convolvedImage]

# ╔═╡ 62dd20ca-6d59-4ef6-8e9b-9b0a5eda118a
convolve(image, kernel)

# ╔═╡ b64a9c4c-5619-4e74-8d35-913850e91184
function brightness(img)
	mean((img.r + img.g + img.b))
end

# ╔═╡ d4784925-f872-4cd2-8e76-8ddb9f3d7dff
function brightness2(img)
	gray_img = Gray.(abs.((img)))
end
...
# ╔═╡ ea21dbf8-ac80-433c-ab07-3beb834f626c
edges(image)
...
# ╔═╡ 63a58fd4-910c-402b-afaf-dbeb8c5d9105
function edges2(img)
	Sy, Sx = Kernel.sobel()
	b = brightness2.(img)
...
[
	convolve(image, Sx) convolve(image, Sy) 
	edges2(image) edges(image)
]

# ╔═╡ dcf82631-4952-4ac2-b262-763481554533
edges2(image)

# ╔═╡ c92f0304-c0a2-45c8-bd14-f6a289d805ff
edges2(wave)

# ╔═╡ 67726a6b-8328-4b4d-b83e-f206f4d910e0
edge = edges2(image2)

# ╔═╡ 0396b46a-7070-498b-8c59-605f4fe30a9d
function DrawSeam(img, seam)
	seam_img = copy(img)
...
function FindSeamAt_Y(directions, position) # position of where the seam starts
	seam = zeros(Int, size(directions)[1]) # 1 for vertical direction 2 for horizontal
...
DrawSeam(img,  FindSeamAt_Y(LeastEnergyMap_Y(edges(img))[2], position))
...
lowest_energy_seam = DrawSeam(image2, FindSeam_Y(LeastEnergyMap_Y(edges(image2))[1]))

# ╔═╡ 4735c79e-00cc-45f1-b367-d2aba418985c
function RemoveSeam_Y(image, seam)
	resolution = (size(image)[1], size(image)[2]-1)
	new_image = Array{RGB}(undef, resolution)
		#fill(RGB{N0f8}(0, 0, 0), size(image)[1], size(image)[2]-1)
...
function SeamCarving_Y(img, width)
	if width < 0 || width > size(img)[2]
		return "width not in bounds of the image"
	end
	
	for i = (1:size(img)[2]-width)#1:width	
		energy = edges(img)
...
# ╔═╡ 216a6591-3e6f-4d98-9f49-fa93c74ca398
carved_image = SeamCarving_Y(image_to_carve, width)

# ╔═╡ 119794d4-5284-4706-9c2a-8b2d81b85852
[image_to_carve carved_image]
...
manifest_format = "2.0"
project_hash = "05bb9fa5bfb3d1e5ac5a1a84b6bf643b72c6fd51"
